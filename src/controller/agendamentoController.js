...
class AgendamentoController {
  static adicionarAgendamento(req, res) {
    const { pacienteId, profissionalId, dataAgendamento, horaAgendamento, tipoAgendamento, observacoes } = req.body;

    if (!pacienteId || !profissionalId || !dataAgendamento || !horaAgendamento || !tipoAgendamento) {
      return res.status(400).json({ message: 'Preencha todos os campos obrigatórios.' });
    }

    Agendamento.adicionar({ pacienteId, profissionalId, dataAgendamento, horaAgendamento, tipoAgendamento, observacoes}, (err, result) => {
      if (err) {
  ...
  static visualizarAgendamentos(req, res) {
    Agendamento.visualizarTodos((err, results) => {
      ...
      res.json({ agendamento: results });
    });
  }

  static visualizarPorId(req, res) {
      const { id } = req.params;
